...
if ( ! class_exists( 'mastery_Notify_System' ) ) {
	/**
	 * Class mastery_Notify_System
	 */
	class mastery_Notify_System {
		/**
		 * @param $ver
		...
		public static function version_check( $ver ) {
			...
			return version_compare( $mastery['Version'], $ver, '>=' );
		}
		...
		public static function has_posts() {
			...
			$imported = get_option( 'mastery_importded_demo' );
			if ( $imported ) {
				return true;
			}
			...
			if ( $count_posts->publish > 4 ) {
				add_option( 'mastery_importded_demo', true );
				return true;
			}
		...
		public static function widget_importer_exporter_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: Widget Importer Exporter Plugin', 'mastery' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Activate: Widget Importer Exporter Plugin', 'mastery' );
			}
		...
		public static function wordpress_importer_title() {
			...
			if ( ! $installed ) {
				return __( 'Install: WordPress Importer', 'mastery' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Activate: WordPress Importer', 'mastery' );
			}
		...
		public static function wordpress_importer_description() {
			$installed = self::check_plugin_is_installed( 'wordpress-importer' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress Importer to create the demo content.', 'mastery' );
			}

			$active = self::check_plugin_is_active( 'wordpress-importer' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Importer to create the demo content.', 'mastery' );
			}
		...
		public static function widget_importer_exporter_description() {
			$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
			if ( ! $installed ) {
				return __( 'Please install the WordPress widget importer to create the demo content', 'mastery' );
			}

			$active = self::check_plugin_is_active( 'widget-importer-exporter' );
			if ( $installed && ! $active ) {
				return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'mastery' );
			}
